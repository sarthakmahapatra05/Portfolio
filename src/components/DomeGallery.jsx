...
import wa0 from '../images/WhatsApp .jpeg';
import wa1 from '../images/WhatsApp Image 2025-12-24 .jpeg';
import wa2 from '../images/WhatsApp Image 2025-12-24 at 02.06.59.jpeg';
...
const DEFAULT_IMAGES = [
  ...
  { src: wa0, alt: 'WhatsApp 1' },
  { src: wa1, alt: 'WhatsApp 2' },
...
const getDataNumber = (el, name, fallback) => {
  const attr = (el && (el.dataset?.[name] ?? el.getAttribute(`data-${name}`))) || null;
...
export default function DomeGallery(props) {
  ...
  useGesture({
    onDragStart: ({ event }) => {
      ...
      const potential = event.target?.closest?.('.item__image') || null;
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div key={`${it.x},${it.y},${i}`} className="sphere-item absolute m-auto" data-src={it.src} data-alt={it.alt} data-offset-x={it.x} data-offset-y={it.y} data-size-x={it.sizeX} data-size-y={it.sizeY} style={{ ['--offset-x']: it.x, ['--offset-y']: it.y, ['--item-size-x']: it.sizeX, ['--item-size-y']: it.sizeY, top: '-999px', bottom: '-999px', left: '-999px', right: '-999px' }}>
                  <div className="item__image absolute block overflow-hidden cursor-pointer bg-gray-200 transition-transform duration-300" role="button" tabIndex={0} aria-label={it.alt || 'Open image'} onClick={e => { if (draggingRef.current) return; if (movedRef.current) return; if (performance.now() - lastDragEndAt.current < 80) return; if (openingRef.current) return; openItemFromElement(e.currentTarget); }} onPointerUp={e => { if (e.nativeEvent.pointerType !== 'touch') return; if (draggingRef.current) return; if (movedRef.current) return; if (performance.now() - lastDragEndAt.current < 80) return; if (openingRef.current) return; openItemFromElement(e.currentTarget); }} style={{ inset: '10px', borderRadius: `var(--tile-radius, ${imageBorderRadius})`, backfaceVisibility: 'hidden' }}>
